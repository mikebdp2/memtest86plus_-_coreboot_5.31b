...
int mt86_strlen(const char *string)
{
	int i = 0;
...
int mt86_strstr(const char *haystack, const char *needle)
{
	int haystack_len = mt86_strlen(haystack);
	int needle_len = mt86_strlen(needle);
	int max_idx = haystack_len - needle_len;
	int idx;

	for (idx = 0; idx <= max_idx; idx++) {
		if (mt86_memcmp(haystack + idx, needle, needle_len) == 0)
			return idx;
	}
	return -1;
}
...
void serial_echo_init(void)
{
	...
	serial_div = (port->refclk / port->baudrate) / 16;
...
void serial_console_setup_from_lb_serial(const struct lb_serial *serial)
{
	...
	console_serial.refclk = serial->input_hertz;
}
